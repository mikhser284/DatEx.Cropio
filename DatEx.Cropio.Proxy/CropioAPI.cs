...
namespace DatEx.Cropio.Proxy
{
    ...
    public class CropioApi : CropioBaseAPi
    {
        ...
        private static String _CropioAgentConfigurationDefault = $"../CropioAgentConfiguration.json";
        private static String _CropioAgentConfigurationSecret = $"../{_CropioAgentConfigurationDefault}";

        public static CropioApi CreateUsingConfigFile()
        {
            string workDir = Environment.CurrentDirectory;
            String configurationFilePath = File.Exists(_CropioAgentConfigurationSecret) 
                ? _CropioAgentConfigurationSecret 
                : _CropioAgentConfigurationDefault;
            if(!File.Exists(configurationFilePath)) throw new FileNotFoundException(configurationFilePath);
            AgentConnectionData config = null;
            using(StreamReader file = File.OpenText(configurationFilePath))
            {
                JsonSerializer serializer = new JsonSerializer();
                config = (AgentConnectionData)serializer.Deserialize(file, typeof(AgentConnectionData));
            }
            return new CropioApi(config.CropioServerAddress, config.Login, config.Password);
        }
